...
export class ConnectionManagerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.rotationService.setupRotationInterval(() => this.isShuttingDown);
  ...
  private clearIntervals() {
    ...
    this.rotationService.clearRotationInterval();
  ...
  public getTelegramClient(mobile: string): TelegramClient | null {
    this.logger.debug(`🔍 Getting Telegram client for mobile: ${mobile}`);
    const client = this.clienManagementService.getTelegramClient(mobile);
    if (client) {
  ...
  public getManagedMobiles(): string[] {
    const mobiles = this.clienManagementService.getManagedMobiles();
    this.logger.debug(`📱 Retrieved ${mobiles.length} managed mobiles`);
    return mobiles;
  }

  public getCurrentActiveMobiles(): string[] {
    const mobiles = this.rotationService.getCurrentActiveMobiles();
    this.logger.debug(`🎯 Retrieved ${mobiles.length} currently active mobiles`);
    return mobiles;
  }

  public getActiveTelegramClients(): Map<string, TelegramClient> {
    const clients = this.rotationService.getActiveTelegramClients();
    this.logger.debug(`🔌 Retrieved ${clients.size} active Telegram clients`);
    return clients;
  }

  public getAllManagedTelegramClients(): Map<string, TelegramClient> {
    const clients = this.clienManagementService.getAllManagedTelegramClients();
    this.logger.debug(`📊 Retrieved ${clients.size} total managed Telegram clients`);
    return clients;
  }
  ...
  public async forceRotateActiveMobiles(): Promise<void> {
    ...
    await this.rotationService.forceRotateActiveMobiles();
  ...
  public getServiceStatistics(): ServiceStatistics {
    ...
    const clients = this.clienManagementService.getManagedClientsMap();
    ...
    const statistics = {
      totalManagedClients: clients.size,
    ...
    this.logger.debug(`📈 Service statistics calculated:`, {
      totalManagedClients: statistics.totalManagedClients,
  ...
  public isServiceReady(): boolean {
    const clientsSize = this.clienManagementService.getManagedClientsMap().size;
  ...
  public getServiceHealth(): ServiceHealth {
    ...
    if (health.status === 'healthy') {
      this.logger.debug('✅ Service health: All systems operational');
    } else if (health.status === 'degraded') {
      this.logger.warn(`⚠️ Service health degraded: ${issues.join(', ')}`);
    } else {
      this.logger.error(`🚨 Service health critical: ${issues.join(', ')}`);
    }
